...
int dev_fill_forward_path_continue(struct net_device_path_ctx *ctx,
...
EXPORT_SYMBOL_GPL(dev_fill_forward_path_continue);

int dev_fill_bridge_path(const struct net_device *dev, const u8 *daddr,
			  ...
			  struct net_device_path_stack *stack)
{
	...
	return dev_fill_forward_path_continue(&ctx, stack);
}

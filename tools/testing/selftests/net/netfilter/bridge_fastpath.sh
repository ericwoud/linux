...
#
# The second series, in the bridge-fastpath, using a vlan-unaware bridge.
# Here we test all combinations of using vlan devices as port slaves.
#
# The third series, in the bridge-fastpath, using a vlan-aware bridge.
...
#
# The fifth series, in the forward-fastpath, using a vlan-aware bridge,
...
add_zone()
{
	...
	if ip -net "$nsrt" link show dev "$dev1.$VID1" 1>/dev/null 2>/dev/null; then
		dev1="$dev1.$VID1"
	fi
	if ip -net "$nsrt" link show dev "$dev2.$VID1" 1>/dev/null 2>/dev/null; then
		dev2="$dev2.$VID1"
	fi
	...
	if [[ $family == "bridge" ]]; then
		...
		table ${family} filter {
		    chain preroutingzones {
		        ...
		        iif { $dev1 , $dev2 } ct zone set 23 # For tags other then single vlan (bridged)
		    }
		}
		EOF
	fi
...
setup_nftables()
{
	local i1=$1
	local i2=$2
	local dev1="${vethrt[$i1]}"
	local dev2="${vethrt[$i2]}"
...
test_unaware_bridge_vlan()
{
	local lret=0
	local i

	ip -net "$nsrt" link add link "${vethrt[$LAN1]}" name "${vethrt[$LAN1]}.$VID1" type vlan id $VID1
	ip -net "$nsrt" link set "${vethrt[$LAN1]}.$VID1" master $BRLAN up
	set_client "$LAN1" q
	set_client "$LAN2" none

	test_paths $LAN1 $LAN2 "unaware bridge, with/without vlan device,  "
	lret=$((lret | $?))

	ip -net "$nsrt" link add link "${vethrt[$LAN2]}" name "${vethrt[$LAN2]}.$VID1" type vlan id $VID1
	ip -net "$nsrt" link set "${vethrt[$LAN2]}.$VID1" master $BRLAN up
	set_client "$LAN2" q

	test_paths $LAN1 $LAN2 "unaware bridge, with/with vlan device,     "
	lret=$((lret | $?))

	ip -net "$nsrt" link del "${vethrt[$LAN1]}.$VID1"
	set_client "$LAN1" none

	test_paths $LAN1 $LAN2 "unaware bridge, without/with vlan device,  "
	lret=$((lret | $?))

	ip -net "$nsrt" link del "${vethrt[$LAN2]}.$VID1"
	for i in $LAN1 $LAN2; do
		unset_client "$i"
	done
	return $lret
...
if [ -n "${tests[3]}" ] || [ -n "${tests[4]}" ] || [ -n "${tests[5]}" ]; then

...
test_path() {
	...
	local lret=0
	local ads=""
	local ipv ad a lpid bytes error checksize
	...
	for ad in $ads; do
		...
		if [ -z "$dofast" ] && [ "$bytes" -lt "$((2 * checksize))" ]; then

			error[i]="established bytes $bytes < $((2 * checksize))"
			continue
		fi
		if [ -n "$dofast" ] && [ "$bytes" -gt "$checksize" ]; then
			# Significant reduction of bytes expected
			error[i]="counted bytes $bytes > $checksize"
			continue
		fi
	done
...
test_paths() {
	...
	if [[ "$desc" == "unaware bridge,"* ]] && [ -z "${tests[2]}" ]; then ############ CHANGE TO !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

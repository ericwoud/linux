...
setup_ns nsclientwan nsclientlan1 nsclientlan2
        ...
        WAN=0        ; LAN1=1        ; LAN2=2          ; ADWAN=3     ; ADLAN=4 ; # shellcheck disable=SC2154
	...
	if [[ $family == "bridge" ]]; then
		...
		table ${family} filter {
		    chain preroutingzones {
		        ...
		        iif ${devs} ct zone set 23 # Needed for tags other then single vlan (bridged)
		    }
		}
		EOF
	fi
...
setup_fastpath()
{
	...
	table ${family} filter {
	    ...
	    chain forward {
	        ...
	        counter name "check"
	        meta l4proto tcp # Needed for tags other then single vlan (bridged)

...
/* Registers on PBUS */
#define EN8811H_PBUS_SW_RESET		0xcf928
#define EN8811H_PBUS_SW_RESET_RELEASE		0x0
...
static int en8811h_pbusdev_init(struct mdio_device *mdiodev)
{
	u32 pbus_value;
	int ret;
	...
	ret = air_pbus_reg_read(mdiodev, EN8811H_PBUS_SW_RESET,
				&pbus_value);
	if (ret < 0)
		return ret;

	/* Internal SW reset of en8811h-pbusdev */
	ret = air_pbus_reg_write(mdiodev, EN8811H_PBUS_SW_RESET,
				 EN8811H_PBUS_SW_RESET_RELEASE);
	if (ret < 0)
		return ret;
...
static int en8811h_probe(struct phy_device *phydev)
{
	...
	ret = en8811h_pbusdev_init(mdiodev);
	if (ret < 0) {
...
static int en8811h_config_init(struct phy_device *phydev)
{
	...
	ret = en8811h_pbusdev_init(priv->pbusdev);
	if (ret < 0)
		return ret;

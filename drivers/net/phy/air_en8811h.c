 ...
 *
 * Source originated from airoha's en8811h.c and en8811h.h v1.2.2
 *
...
/* testing */
#define EN8811H_FLOW_CTRL1		0xe0008
#define EN8811H_FLOW_CTRL1_ENABLE		(BIT(6) | BIT(7))

#define EN8811H_FLOW_CTRL2		0xe000c
#define EN8811H_FLOW_CTRL2_ENABLE		(BIT(0) | BIT(1))
/* end testing */
...
/* Registers on extra device PBUS */
#define EN8811H_EXTRA_SW_RESET		0xcf928
#define EN8811H_EXTRA_SW_RESET_RELEASE		0x0
...
{
	struct device *dev = &phydev->mdio.dev;
	struct device_node *np = dev->of_node; // remove
	struct mdio_device *mdiodev = NULL;
	struct device_node *mdio_node;
	struct en8811h_priv *priv;

	mdio_node = of_parse_phandle(np, "airoha,extradev-handle", 0);
	if (mdio_node) {
		mdiodev = of_mdio_find_device(mdio_node);
		of_node_put(mdio_node);
		if (!mdiodev) {
			phydev_dbg(phydev, "Couldn't find extradev\n");
			return -EPROBE_DEFER;
		}

		if (!mdiodev->dev.driver) {
			phydev_dbg(phydev, "Attached extradev not ready\n");
			put_device(&mdiodev->dev);
			return -EPROBE_DEFER;
		}
		phydev_info(phydev, "Added extradev\n");
	} else {
		phydev_warn(phydev, "airoha,extradev-handle not defined\n");
	}
...
static int en8811h_extradev_init(struct mdio_device *mdiodev)
{
	u32 pbus_value;
	int ret;
	...
	ret = air_pbus_reg_read(mdiodev, EN8811H_EXTRA_SW_RESET,
				&pbus_value);
	if (ret < 0)
		return ret;

	/* Internal SW reset of en8811h-extradev */
	ret = air_pbus_reg_write(mdiodev, EN8811H_EXTRA_SW_RESET,
				 EN8811H_EXTRA_SW_RESET_RELEASE);
	if (ret < 0)
		return ret;

	dev_dbg(&mdiodev->dev, "sw reset released 0x%x->0x%x\n", pbus_value,
		EN8811H_EXTRA_SW_RESET_RELEASE);
...
static int en8811h_config_init(struct phy_device *phydev)
{
	...
	ret = en8811h_extradev_init(priv->extradev);
	if (ret < 0)
		return ret;
...
/* testing */
	ret = air_buckpbus_reg_read(phydev, EN8811H_FLOW_CTRL1, &pbus_value);
	if (ret < 0)
		return ret;
	phydev_info(phydev, "EN8811H_FLOW_CTRL1: 0x%08lx\n",
		    pbus_value & EN8811H_FLOW_CTRL1_ENABLE);

	ret = air_buckpbus_reg_read(phydev, EN8811H_FLOW_CTRL2, &pbus_value);
	if (ret < 0)
		return ret;
	phydev_info(phydev, "EN8811H_FLOW_CTRL2: 0x%08lx\n",
		    pbus_value & EN8811H_FLOW_CTRL2_ENABLE);

	/* Enable flow control */
	ret = air_buckpbus_reg_modify(phydev, EN8811H_FLOW_CTRL1,
				      EN8811H_FLOW_CTRL1_ENABLE,
				      EN8811H_FLOW_CTRL1_ENABLE);
	if (ret < 0)
		return ret;
	ret = air_buckpbus_reg_modify(phydev, EN8811H_FLOW_CTRL2,
				      EN8811H_FLOW_CTRL2_ENABLE,
				      EN8811H_FLOW_CTRL2_ENABLE);
	if (ret < 0)
		return ret;
/* end testing */
...
static int en8811h_extradev_probe(struct mdio_device *mdiodev)
{
	return en8811h_extradev_init(mdiodev);
}
...
/* Without compatible property, it is considered to be a phy device */
static const struct of_device_id en8811h_extradev_of_match[] = {
	{ .compatible = "airoha,en8811h-extradev" },
	{},
};

static struct mdio_driver en8811h_extradev_driver = {
	.probe	= en8811h_extradev_probe,
	.mdiodrv.driver = {
		.name = "en8811h_extradev",
		.of_match_table = en8811h_extradev_of_match,
	},
};

static int __init en8811h_drivers_register(void)
{
	int ret;
	...
	ret = mdio_driver_register(&en8811h_extradev_driver);
	if (ret) {
		mdio_driver_unregister(&en8811h_extradev_driver);
		phy_driver_unregister(en8811h_driver);
		return ret;
	}
...
static void __exit en8811h_drivers_unregister(void)
{
	mdio_driver_unregister(&en8811h_extradev_driver);
	phy_driver_unregister(en8811h_driver);
}

module_init(en8811h_drivers_register);
module_exit(en8811h_drivers_unregister);
...
MODULE_DEVICE_TABLE(of, air_extradev_of_match);
